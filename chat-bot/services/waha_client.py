...
_WAHA_API_KEY = os.getenv("WAHA_API_KEY", "")
...
def _headers() -> dict:
    if _WAHA_API_KEY:
        return {"X-Api-Key": _WAHA_API_KEY}
    return {}
...
def enviar_imagem_waha(chat_id: str, image_url: str):
    ...
    try:
        ...
        resp = requests.post(f"{WAHA_BASE_URL}/api/sendImage", json=payload, headers=_headers(), timeout=15)
        time.sleep(1)
        return resp.json()
    except Exception as e:
